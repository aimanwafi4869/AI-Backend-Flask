...
import numpy as np
import service.QuickDrawService as service
import torch
aiApp = service.QuickDrawService()

class QuickDrawAiClass(object):

    controller = Blueprint("quickdraw", __name__, url_prefix="/api/ai/quickdraw")
    ...
    @controller.route("/preload")
    def preload():
        aiApp.existingModel()
        return{}
    
    @controller.route("/testBlank")
    def testBlank():
        return aiApp.testBlank()
